...
my_plot_ordination <- function(ps) {
...
my_plot_boxes <- function(dat) {
  ## generate plot for panel 4
  plot(c(0,1,2),c(5,4,6))
...
server <- function(input, output) {
  ...
  observeEvent(input$user_disease, {
    ...
    user_phyloseq <- phyloseq(otu_table(user_counts, taxa_are_rows = T), sample_data(read.csv(input$user_disease$datapath)))
    merged_data <- merge_phyloseq(ps,user_phyloseq)
    subsetted_data <- merged_data
    #criteria <- list() ## create a set of subsetting criteria based on user input
    #subsetted_data <- custom_subset_samples(merged_data, criteria)
    p$panel1 <- my_plot_map(merged_data)
    p$panel2 <- my_plot_taxonomy(subsetted_data)
    p$panel3 <- my_plot_ordination(subsetted_data)
    p$panel4 <- my_plot_boxes(subsetted_data)
  })
